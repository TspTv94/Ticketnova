    ...
    card.innerHTML=`
      ...
      <div class="card-body">
        ...
        <div class="card-footer">
          ...
          <button class="book-btn" onclick="openModal('${ev._id || ev.id}')" ${soldOut?'disabled':''}>
...
function openModal(id){
  currentEvent=allEvents.find(e=>(e._id||e.id)==id || e._id===id || String(e.id)===String(id));
...
async function processBooking(){
  ...
  if(!fname||!lname||!email||!phone){showToast('warning','Missing Details','Please fill in all required fields.');return;}
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){showToast('warning','Invalid Email','Enter a valid email.');return;}
  if(card.length<16){showToast('warning','Invalid Card','Enter a valid 16-digit card number.');return;}
  if(expiry.length<5){showToast('warning','Invalid Expiry','Enter MM/YY.');return;}
  if(cvv.length<3){showToast('warning','Invalid CVV','Enter a 3-digit CVV.');return;}

  const selectedTickets=currentEvent.tickets.map((t,i)=>quantities[i]>0?{type:t.type,qty:quantities[i]}:null).filter(Boolean);
  if(!selectedTickets.length){showToast('warning','No Tickets','Select at least one ticket.');return;}
  ...
  if(result.success){
    ...
    const booking={
      ...
      customer:{firstName:fname,lastName:lname,email,phone},
      tickets:currentEvent.tickets.map((t,i)=>quantities[i]>0?{type:t.type,qty:quantities[i],price:t.price}:null).filter(Boolean),
      subtotal:sub,bookingFee:fee,total:sub+fee,
      status:'confirmed',
    ...
    showToast('success','Booking Confirmed','Ticket ID: '+booking.id);
  }else{
...
function renderMyTickets(){
  ...
  if(!myTickets.length){
    list.innerHTML='<div class="empty-state"><div class="empty-icon">🎟️</div><p>No tickets yet.<br>Browse events and book your first experience!</p></div>';
    return;
  }
  list.innerHTML=myTickets.slice().reverse().map(t=>`
    <div class="my-ticket">
      ...
      <div class="ticket-details">
        ...
        <div class="ticket-meta">${t.tickets.map(tk=>`${tk.qty}x ${tk.type}`).join(' | ')} &nbsp;·&nbsp; <span style="color:var(--gold);font-family:'Space Mono',monospace;">₹${t.total.toLocaleString('en-IN')}</span></div>
...
function showToast(type,title,msg){
  ...
  const icons={'success':'🎟️','error':'❌','warning':'⚠️'};
  document.getElementById('toast-icon').textContent=icons[type]||'🎟️';
