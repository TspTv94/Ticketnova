...
const createBooking = (req, res) => {
  try {
    ...
    if (!event.seatsLeft) return res.status(400).json({ success:false, message:'This event is sold out.' });

    let subtotal = 0;
    const ticketDetails = [];
    for (const t of tickets) {
      const tt = event.tickets.find(x => x.type === t.type);
      if (!tt) return res.status(400).json({ success:false, message:`Ticket type "${t.type}" not found.` });
    ...
    const booking = {
      ...
      venue: event.venue, emoji: event.emoji, gradient: event.gradient,
      category: event.category, customer,
      tickets: ticketDetails, subtotal, bookingFee,
...
EOF
console.log("✅ controllers/bookingsController.js done");